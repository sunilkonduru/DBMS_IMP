...
public:
	...
	RC getValueInTuple(void *data,vector<Attribute> attrs,string attrName,AttrType &type,Value &value);
	RC CompareValue(void *leftData,vector<Attribute> leftAttrs,void *rightData,vector<Attribute> rightAttrs,Condition condition);
	RC sizeOfTuple(void *data,vector<Attribute> attrs,int &size);
	RC populateAttribute(Iterator *_leftIn,Iterator *_rightIn,vector<Attribute> &_attrs);
...
class NLJoin : public Iterator {
    ...
    public:
		...
		RM *_rm;
		vector<Attribute> _attrs;
		vector<Value> leftValues;
		vector<Value> rightValues;
		int _LIndex;
		int _RIndex;
...
class HashJoin : public Iterator {
    ...
    public:
        ...
        ~HashJoin();
        RC writepage(string filename,HashPage page,int &pageId);
        RC readBucket(string filename,HashPage &page,vector<int> pageNums);
        RC readpage(string filename,HashPage &page,int pageId);
        RC buildHashTable(Iterator *In,vector<HashPage> &pages,string attrname);
        int HashFunction1(void *data,AttrType type);
        int HashFunction2(void *data,AttrType type);
        RC process();

        RC getHashFileName(string &attrname);
...
class Aggregate : public Iterator {
    ...
    public:
        ...
        RC conditionOperation(float &result,vector<float> vals);
};
